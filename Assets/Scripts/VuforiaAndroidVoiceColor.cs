...
{
    [Header("Contenuto AR")]
    public GameObject contenuto3D;

    [Header("Comandi vocali")]
    public string comandoCambioColore = "cambia colore";
    ...
    [Header("Colori disponibili")]
    public Color[] colori = new Color[]
    {
        Color.red,
        Color.blue,
        Color.green,
    ...
    private void CambiaColore()
    {
        ...
        Color nuovoColore = colori[indiceColore];
        ImpostaColore(nuovoColore);

        indiceColore++;

        if (indiceColore >= colori.Length)
        {
            indiceColore = 0;
        }
    ...
    private void ImpostaColore(Color colore)
    {
        ...
        foreach (Renderer renderer in renderers)
        {
            foreach (Material material in renderer.materials)
            {
                ...
                {
                    material.SetColor("_BaseColor", colore); // URP / HDRP
                }
                else if (material.HasProperty("_Color"))
                {
                    material.SetColor("_Color", colore); // Standard Shader
                }
            }
        }
    }
    ...
    private void EseguiComandoVocale(string testo)
    {
        ...
        if (testo.Contains(comandoCambioColore))
        {
            CambiaColore();
        }
        else if (testo.Contains("rosso"))
        {
            ImpostaColore(Color.red);
        }
        else if (testo.Contains("blu"))
        {
            ImpostaColore(Color.blue);
        }
        else if (testo.Contains("verde"))
        {
            ImpostaColore(Color.green);
        }
        else if (testo.Contains("giallo"))
        {
